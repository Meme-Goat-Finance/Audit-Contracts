...
;; LAUNCHPAD DATA
;; data is filled on contract creation
(define-constant LISTING-ALLOCATION u0)
(define-constant SALE-ALLOCATION u0)
(define-constant LAUNCHPAD-TOKEN .sip-10-token)
...
(define-constant MIN-BUY u0)
(define-constant MAX-BUY u0)
(define-constant HARD-CAP u0) 
(define-constant SOFT-CAP u0)
(define-constant START-BLOCK u0)
(define-constant END-BLOCK u0)
...
(define-public (claim-token (token-trait <ft-trait>)) 
  (let
    ...
    (try! (check-is-initialized))
    (asserts! (or (< END-BLOCK block-height) (is-eq stx-pool-balance HARD-CAP)) ERR-TOKEN-LAUNCH-NOT-ENDED)
...
(define-private (initialize)
  ...
  (begin
    ...
    (try! (contract-call? .sip-10 transfer LISTING-ALLOCATION tx-sender LISTING-SIG-WALLET none))
    (try! (contract-call? .sip-10 transfer SALE-ALLOCATION tx-sender LAUNCHPAD-ADDRESS none))
